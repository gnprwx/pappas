...
With the back end ready to go, I created a component with the fetch statement. That's when I learned about **useEffect**. From what I understand, it'll render changes in the useState variable **data** inside the fetchData function, making the changes available in our return statement. With useEffect and useState, we get the same functionality we'd expect in JavaScript in fetching data, but with the bonus of automatically re-rendering that portion of the DOM.
